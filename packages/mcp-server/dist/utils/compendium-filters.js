 ...
 */
export const PF2eRarities = ['common', 'uncommon', 'rare', 'unique'];
/**
 ...
 */
export function getFilterSchema(system) {
    if (system === 'dnd5e') {
 ...
 */
export function isValidCreatureType(creatureType, system) {
    if (system === 'dnd5e') {
        return DnD5eCreatureTypes.includes(creatureType);
    }
    else if (system === 'pf2e') {
        return PF2eCreatureTypes.includes(creatureType);
    }
    return false;
}
/**
 ...
 */
export function convertFilters(filters, fromSystem, toSystem) {
 ...
 */
export function describeFilters(filters, system) {
    const parts = [];
    if (system === 'dnd5e') {
        if (filters.challengeRating !== undefined) {
        ...
        if (filters.creatureType)
            parts.push(filters.creatureType);
        if (filters.size)
            parts.push(filters.size);
        if (filters.alignment)
            parts.push(filters.alignment);
        ...
        if (filters.hasSpells)
            parts.push('spellcaster');
    }
    return parts.length > 0 ? parts.join(', ') : 'no filters';
}
//# sourceMappingURL=compendium-filters.js.map