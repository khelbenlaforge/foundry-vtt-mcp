 ...
 */
export function isValidComfyUIPath(dirPath) {
    try {
 ...
 */
function getCommonComfyUIPaths() {
    const paths = [];
    const home = process.env.HOME || process.env.USERPROFILE || '';
    if (isWindows()) {
        const localAppData = process.env.LOCALAPPDATA || 'C:\\Users\\Default\\AppData\\Local';
        // Windows paths
        paths.push(`${localAppData}\\FoundryMCPServer\\ComfyUI-headless`, `${localAppData}\\ComfyUI`, 'C:\\ComfyUI', `${home}\\ComfyUI`);
    }
    else if (isMac()) {
        const appSupport = `${home}/Library/Application Support`;
        // Mac paths - prioritize headless installer using system Python
        paths.push('/Applications/FoundryMCPServer.app/Contents/Resources/ComfyUI', // Headless install (uses system Python)
        `${appSupport}/FoundryMCPServer/ComfyUI-headless`, `${appSupport}/ComfyUI`, '/Applications/ComfyUI.app/Contents/Resources/ComfyUI', // Desktop app (legacy)
        `${home}/ComfyUI`, '/opt/ComfyUI', '/usr/local/ComfyUI');
    }
    else {
        // Linux paths
        paths.push(`${home}/.local/share/FoundryMCPServer/ComfyUI-headless`, `${home}/ComfyUI`, '/opt/ComfyUI', '/usr/local/ComfyUI');
    }
    return paths;
}
/**
 ...
 */
export function detectComfyUIInstallation() {
 ...
 */
export function getDefaultPythonCommand(installPath) {
    if (isWindows()) {
...
//# sourceMappingURL=comfyui-paths.js.map