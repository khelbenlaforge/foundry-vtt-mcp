...
export class SocketBridge {
    ...
    async handleJobCompleted(data) {
        try {
            ...
            const scene = await globalThis.Scene.create(sceneData);
    ...
    async createSceneWalls(scene, wallsData) {
        ...
        try {
            ...
            const validWalls = wallsData.filter((wall) => {
                ...
                if (!wall.c.every((coord) => typeof coord === 'number' && !isNaN(coord))) {
     ...
     */
    async ensureAIMapsFolderExists() {
        try {
            const folderName = 'AI Generated Maps';
            // Check if folder already exists
            const existingFolder = globalThis.game.folders.find((f) => f.type === 'Scene' && f.name === folderName);
            if (existingFolder) {
