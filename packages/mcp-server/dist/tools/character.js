...
export class CharacterTools {
    ...
    async handleGetCharacterEntity(args) {
        const schema = z.object({
    ...
    async handleSearchCharacterItems(args) {
        const schema = z.object({
            characterIdentifier: z.string().min(1, 'Character identifier cannot be empty'),
    ...
    async formatCharacterResponse(characterData) {
        const response = {
            id: characterData.id,
    ...
    formatSpellcasting(spellcastingEntries) {
    ...
    async extractStats(characterData) {
