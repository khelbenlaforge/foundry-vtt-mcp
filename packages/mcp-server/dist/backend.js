...
async function handleGenerateMapRequest(message, jobQueue, comfyuiClient, logger, foundryClient) {
    try {
...
async function handleCancelMapJobRequest(data, jobQueue, comfyuiClient, logger) {
    try {
        if (!data) {
            throw new Error('Request data is required');
        }
...
async function processMapGenerationInBackend(jobId, jobQueue, comfyuiClient, logger, foundryClient) {
    ...
    try {
        ...
        comfyuiClient.registerProgressCallback(comfyuiJob.prompt_id, (progress) => {
...
async function startBackend() {
    ...
    try {
        ...
        if (mapGenerationComfyUIClient && mapGenerationComfyUIClient.config?.autoStart) {
