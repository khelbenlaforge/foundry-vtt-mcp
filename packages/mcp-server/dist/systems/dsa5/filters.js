 ...
 */
export const ExperienceLevels = [1, 2, 3, 4, 5, 6, 7];
/**
 * DSA5 filter schema
 */
 ...
 */
export function matchesDSA5Filters(creature, filters) {
    ...
    if (filters.experiencePoints !== undefined) {
        ...
        if (typeof filters.experiencePoints === 'number') {
            if (ap !== filters.experiencePoints)
                return false;
        }
        else {
 ...
 */
export function describeDSA5Filters(filters) {
    const parts = [];
    if (filters.level !== undefined) {
        if (typeof filters.level === 'number') {
    ...
    if (filters.species)
        parts.push(filters.species);
    if (filters.culture)
        parts.push(filters.culture);
    if (filters.size)
 ...
 */
export function isValidDSA5Species(species) {
    return DSA5Species.includes(species.toLowerCase());
}
/**
