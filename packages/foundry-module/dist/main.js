...
class FoundryMCPBridge {
    ...
    async initialize() {
        try {
            ...
            window.foundryMCPBridge.dataAccess = this.queryHandlers.dataAccess;
     ...
     */
    async checkAndBuildEnhancedIndex() {
        try {
            // Only for GM users
            if (!this.isGMUser())
                return;
        ...
        try {
            ...
            if (this.isGMUser()) {
                ...
                if (errorMessage.includes('ECONNREFUSED') || errorMessage.includes('connect ECONNREFUSED')) {
                    ...
                    const lastShown = this.settings.getSetting('lastMCPServerNotification');
                    const now = new Date().getTime();
                    const thirtySecondsAgo = now - (30 * 1000);
                    if (!lastShown || new Date(lastShown).getTime() < thirtySecondsAgo) {
                        ui.notifications?.warn('MCP Server not found. Install it from https://github.com/adambdooley/foundry-vtt-mcp');
...
window.foundryMCPBridge = foundryMCPBridge;
// Foundry VTT Hooks
                ...
                if (data.type === 'requestMessageUpdate' && data.buttonId && data.messageId) {
                    ...
                    if (game.user?.isGM) {
                        try {
                            ...
                            const queryHandlers = foundryMCPBridge['queryHandlers'];
        ...
        if (rollButtons) {
            ...
            if (queryHandlers && queryHandlers.dataAccess) {
                ...
                for (const [_buttonId, buttonData] of Object.entries(rollButtons)) {
                    if (buttonData && typeof buttonData === 'object' && buttonData.rolled) {
                        break;
                    }
                }
