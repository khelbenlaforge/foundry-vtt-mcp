...
function parseSpeed(speedStr: string): Record<string, number> {
  ...
  for (const part of parts) {
    ...
    if (fly)    result.fly    = parseInt(fly[1]);
    else if (swim)   result.swim   = parseInt(swim[1]);
    else if (climb)  result.climb  = parseInt(climb[1]);
    else if (burrow) result.burrow = parseInt(burrow[1]);
    else if (walk)   result.walk   = parseInt(walk[1]);
  }
  return result;
}

function parseSenses(sensesStr: string): { senses: string[]; passive: number } {
  const parts = sensesStr.split(',').map(s => s.trim());
  const senses: string[] = [];
  let passive = 10;
  for (const part of parts) {
    const pm = part.match(/passive\s+perception\s+(\d+)/i);
    if (pm) passive = parseInt(pm[1]);
    else senses.push(part.toLowerCase());
  }
  return { senses, passive };
}

// Fantasy Statblocks saves/skills are: [{wis: 7}, {cha: 5}]
function parseSavesOrSkills(arr: Array<Record<string, number>>): Record<string, string> {
...
function mapEntries(items: Array<{ name: string; desc: string }>) {
...
function parseStatblock(rawYaml: string): Record<string, any> {
  ...
  // AC
  const acEntry: Record<string, any> = { ac: parsed.ac };
  if (parsed.ac_class) acEntry.from = [parsed.ac_class];
  entry.ac = [acEntry];
  ...
  if (parsed.damage_resistances) {
    entry.resist = String(parsed.damage_resistances).split(',').map((s: string) => s.trim().toLowerCase());
  }
  if (parsed.damage_immunities) {
    entry.immune = String(parsed.damage_immunities).split(',').map((s: string) => s.trim().toLowerCase());
  }
  if (parsed.condition_immunities) {
    entry.conditionImmune = String(parsed.condition_immunities).split(',').map((s: string) => s.trim().toLowerCase());
  }

  // Senses + passive perception
  if (parsed.senses) {
    const { senses, passive } = parseSenses(String(parsed.senses));
    if (senses.length) entry.senses = senses;
  ...
  // Languages
  if (parsed.languages) {
    entry.languages = String(parsed.languages).split(',').map((s: string) => s.trim());
  }
...
export class CreatureImportTools {
  ...
  async handleImportCreatureFromVault(args: unknown) {
    ...
    if (!rawYaml) throw new Error(`No statblock codeblock found in: ${filePath}`);
