import { z } from 'zod';
import { detectGameSystem, getCreatureLevel, getCreatureType, hasSpellcasting } from '../utils/system-detection.js';
...
export class CompendiumTools {
    ...
    systemRegistry;
    gameSystem = null;
    constructor({ foundryClient, logger, systemRegistry }) {
    ...
    async handleSearchCompendium(args) {
    ...
    async handleGetCompendiumItem(args) {
        const schema = z.object({
    ...
    async handleListCreaturesByCriteria(args) {
        // Detect game system for appropriate filtering
        const gameSystem = await this.getGameSystem();
        ...
        try {
            ...
            return {
                ...
                creatures: (results.response?.creatures || results).map((creature) => this.formatCreatureListItem(creature, gameSystem)),
    ...
    async handleListCompendiumPacks(args) {
        const schema = z.object({
        ...
        try {
            ...
            const filteredPacks = type
                ? packs.filter((pack) => pack.type === type)
                : packs;
            ...
            if (gameSystem) {
                ...
                if (creatureType) {
                    if (gameSystem === 'pf2e' && Array.isArray(creatureType)) {
                        ...
                        const primaryType = creatureType.find((t) => creatureTraits.includes(t.toLowerCase()));
                        if (primaryType)
    ...
    formatDetailedCompendiumItem(item) {
    ...
    createItemSummary(item) {
        ...
        switch (item.type.toLowerCase()) {
            case 'spell':
                if (system.level)
                    parts.push(`Level ${system.level}`);
                if (system.school)
                    parts.push(system.school);
                break;
            case 'weapon':
            ...
            case 'armor':
                if (system.armor?.value)
                    parts.push(`AC ${system.armor.value}`);
                break;
            case 'equipment':
            case 'item':
                if (system.rarity)
                    parts.push(system.rarity);
        ...
        if (gameSystem) {
            ...
            if (creatureType) {
                if (gameSystem === 'pf2e' && Array.isArray(creatureType)) {
                    ...
                    const primaryType = creatureType.find((t) => creatureTraits.includes(t.toLowerCase()));
                    if (primaryType)
                        formatted.creatureType = primaryType;
                }
                else {
     ...
     */
    describeCriteria(params, gameSystem) {
        const parts = [];
        if (gameSystem === 'dnd5e') {
    ...
    extractCompactStats(item) {
        ...
        if (system.details?.type?.value)
            stats.creatureType = system.details.type.value;
        if (system.traits?.size)
            stats.size = system.traits.size;
        if (system.details?.alignment)
            stats.alignment = system.details.alignment;
        ...
        if (system.attributes?.movement) {
            ...
            const speeds = [];
            if (movement.walk)
                speeds.push(`${movement.walk} ft`);
            if (movement.fly)
                speeds.push(`fly ${movement.fly} ft`);
            if (movement.swim)
                speeds.push(`swim ${movement.swim} ft`);
            if (speeds.length > 0)
                stats.speed = speeds.join(', ');
        }
        return stats;
    }
    extractItemProperties(item) {
        ...
        if (system.rarity)
            properties.rarity = system.rarity;
        if (system.price)
            properties.price = system.price;
        if (system.weight)
            properties.weight = system.weight;
        if (system.quantity)
            properties.quantity = system.quantity;
        // Spell-specific properties
        if (item.type.toLowerCase() === 'spell') {
            if (system.level !== undefined)
                properties.spellLevel = system.level;
            if (system.school)
                properties.school = system.school;
            if (system.components)
                properties.components = system.components;
            if (system.duration)
                properties.duration = system.duration;
            if (system.range)
                properties.range = system.range;
        }
        // Weapon-specific properties
        if (item.type.toLowerCase() === 'weapon') {
            if (system.damage)
                properties.damage = system.damage;
            if (system.weaponType)
                properties.weaponType = system.weaponType;
            if (system.properties)
                properties.weaponProperties = system.properties;
        }
        // Armor-specific properties
        if (item.type.toLowerCase() === 'armor') {
            if (system.armor)
                properties.armorClass = system.armor;
            if (system.stealth)
                properties.stealthDisadvantage = system.stealth;
        }
        return properties;
    }
