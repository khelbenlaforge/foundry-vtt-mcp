...
export class ModuleSettings {
    ...
    registerSettings() {
        ...
        game.settings.registerMenu(this.moduleId, 'enhancedIndexMenu', {
            ...
            type: class extends FormApplication {
                ...
                activateListeners(html) {
                    ...
                    html.find('.rebuild-index-btn').click(() => {
                        const bridge = globalThis.foundryMCPBridge;
        ...
        game.settings.registerMenu(this.moduleId, 'mapGenerationSettings', {
     ...
     */
    updateConnectionStatusDisplay(connected, _toolCount) {
        try {
            const statusText = connected
                ? `✅ Connected`
                : `❌ Disconnected - Use connection panel to connect`;
            // Update the hint for the enabled setting to show status
            const enabledSetting = game.settings.settings.get(`${this.moduleId}.enabled`);
            if (enabledSetting) {
    ...
    async resetToDefaults() {
        ...
        for (const key of settingKeys) {
            ...
            const setting = game.settings.settings.get(`${this.moduleId}.${key}`);
