...
export class DSA5IndexBuilder {
    ...
    async buildIndex(packs, force = false) {
        ...
        try {
            ...
            for (let i = 0; i < actorPacks.length; i++) {
                ...
                if (typeof ui !== 'undefined' && ui.notifications) {
                    progressNotification = ui.notifications.info(`Erstelle DSA5 Index: Paket ${currentPack}/${actorPacks.length} (${pack.metadata.label})...`);
                }
                try {
    ...
    extractCreatureData(doc, pack) {
        try {
            ...
            const hasSpells = !!(hasAstralEnergy ||
                hasKarmaEnergy ||
                system.spells ||
                system.liturgies ||
