...
function getAppDataDir() {
  ...
  switch (platform) {
    case "win32":
      return process.env.LOCALAPPDATA ? `${process.env.LOCALAPPDATA}\\FoundryMCPServer` : "C:\\Users\\Default\\AppData\\Local\\FoundryMCPServer";
    case "darwin":
...
var ConfigSchema = external_exports.object({
  ...
  logFormat: external_exports.enum(["json", "simple"]).default("simple"),
  enableFileLogging: external_exports.boolean().default(false),
  ...
  foundry: external_exports.object({
    ...
    connectionTimeout: external_exports.number().min(1e3).max(6e4).default(1e4),
    connectionType: external_exports.enum(["websocket", "webrtc", "auto"]).default("auto"),
    protocol: external_exports.enum(["ws", "wss"]).default("ws"),
    // Legacy, used only for WebSocket mode
    remoteMode: external_exports.boolean().default(false),
    dataPath: external_exports.string().optional(),
    // Custom path for generated maps (remote mode)
    rejectUnauthorized: external_exports.boolean().default(true),
    // TLS certificate validation
    // WebRTC configuration
    webrtc: external_exports.object({
      stunServers: external_exports.array(external_exports.string()).default([
  ...
  comfyui: external_exports.object({
    // ComfyUI always runs locally on the same machine as the MCP server
    port: external_exports.number().min(1024).max(65535).default(31411),
    installPath: external_exports.string(),
    // No default here - set in rawConfig
    host: external_exports.string().default("127.0.0.1"),
    pythonCommand: external_exports.string().default("python/python.exe")
    // Will be platform-specific
  }),
  toolResponseMaxChars: external_exports.number().min(256).max(5e5).default(2e4),
  server: external_exports.object({
    name: external_exports.string().default("foundry-mcp-server"),
    version: external_exports.string().default("0.4.17")
  })
});
var rawConfig = {
  ...
  foundry: {
    ...
    connectionType: process.env.FOUNDRY_CONNECTION_TYPE || "auto",
    protocol: process.env.FOUNDRY_PROTOCOL || "ws",
    ...
    webrtc: {
      stunServers: process.env.FOUNDRY_STUN_SERVERS ? process.env.FOUNDRY_STUN_SERVERS.split(",") : ["stun:stun.l.google.com:19302", "stun:stun1.l.google.com:19302"]
    }
  },
  comfyui: {
...
var BackendClient = class {
  ...
  log(msg, meta) {
    try {
      ...
      const line = `[${(/* @__PURE__ */ new Date()).toISOString()}] ${msg}${meta ? " " + JSON.stringify(meta) : ""}
`;
  ...
  connect() {
    return new Promise((resolve, reject) => {
      ...
      sock.on("error", (e) => {
        this.log("connect(): error", { error: e?.message });
        reject(e);
      });
    });
  }
  async connectWithRetry() {
    try {
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        const delayMs = Math.min(250 * Math.pow(1.4, attempt), 2e3);
  ...
  startBackend() {
    return new Promise(async (resolve) => {
      ...
      try {
        const backendUrl = new URL("./backend.js", "bundled");
        backendPath = (0, import_url.fileURLToPath)(backendUrl);
      } catch {
        const pathMod = await import("path");
        const fsMod = await import("fs");
        const baseDir = typeof __dirname !== "undefined" ? __dirname : pathMod.dirname(process.argv && process.argv[1] || process.cwd());
      ...
      const child = (0, import_child_process.spawn)(process.execPath, [backendPath], {
        detached: false,
        // Stay attached to monitor backend
        stdio: ["ignore", "ignore", "pipe"]
        // Capture stderr to detect exit
      });
