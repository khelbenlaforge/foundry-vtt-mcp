 ...
 */
export function matchesPF2eFilters(creature, filters) {
    // Level filter
    ...
    if (filters.creatureType) {
        ...
        if (!Array.isArray(traits))
            return false;
        const hasType = traits.some((trait) => trait.toLowerCase() === filters.creatureType.toLowerCase());
        if (!hasType)
            return false;
    }
    ...
    if (filters.traits && filters.traits.length > 0) {
        ...
        if (!Array.isArray(creatureTraits))
            return false;
        const lowerTraits = creatureTraits.map((t) => t.toLowerCase());
 ...
 */
export function describePF2eFilters(filters) {
    const parts = [];
    if (filters.level !== undefined) {
        if (typeof filters.level === 'number') {
 ...
 */
export function isValidPF2eCreatureType(creatureType) {
